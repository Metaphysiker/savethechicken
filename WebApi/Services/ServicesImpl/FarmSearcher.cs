...
{
    public class FarmSearcher : IModelSearcher<Farm, FarmSearch>
    {
        private readonly DatabaseContext _db;
        public FarmSearcher(DatabaseContext db)
        {
            _db = db;
        }

        public async Task<PaginationDto<Farm>> SearchAsync(FarmSearch search, params Expression<Func<Farm, object?>>[] includes)
        {
            var query = _db.Set<Farm>().AsQueryable();
